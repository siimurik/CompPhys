...
This will simulate the inflow behaviour into the pipe and the 
...
sceme (very similar to the SIMPLE algorithm). 
...
->  A rectangular domain (think of a slice from a pipe with
    circular cossection alongside the longtitudal axis)
...
The flow is developing the characteristic parabula Hagen-Poiseulle
...
All variables (u-velocity, y-velocity, pressure) are saved
...
outside of it, is called a ghost node. We need it to encforce 
the boundary condition.

Agin, if we have N_x vertex nodes in x direction and N_y vertex nodes
i y direction, then (introducing the ghost nodes):
...
Usage of a P2 pressure correction scheme (very similar to SIMPLE
algorithm)
...
0. Initialization
    ...
    0.3 Initialize the p (=pressure) unifromly with zeros
...
7. Repeat time loop until until steady-state is reached

For visualizations the velocities have to be mapped to the
...
3. The Pressure Poisson (correction) problem is solved using
   ...
   application, but due to the fixed number of iterations, it
   does not ensure that the residual is sufficiently small. 
   That could introduce local compressibility.
"""
...
def main():
    ...
    for iter in tqdm(range(N_TIME_STEPS)):
        diffusion_x = KINEMATIC_VISCOSITY * (
            (
                velocity_x_prev[1:-1, 2:  ]       # [1:-1] all steps in y direction
                +                               # [2:  ] from 2nd to all the way to the end in x dir
        ...
        velocity_x_tent[1:-1, 1:-1] = (
            velocity_x_prev[1:-1, 1:-1]     # u
            +
            TIME_STEP_LENGTH                # dt
            *
            (
                -
                pressure_gradient_x         # ∂p/∂x 
                +
                diffusion_x                 # ν ∇²u
                -
                convection_x                # ∂u²/∂x
        ...
        diffusion_y = KINEMATIC_VISCOSITY * (
            (
                velocity_y_prev[1:-1, 2:  ]     # [1:-1] all steps in y direction
                +                               # [2:  ] from 2nd to all the way to the end in x dir
        ...
        velocity_y_tent[1:-1, 1:-1] = (
            velocity_y_prev[1:-1, 1:-1]     # v
            +
            TIME_STEP_LENGTH                # dt
            *
            (
                -
                pressure_gradient_y         # ∂p/∂y 
                +
                diffusion_y                 # ν ∇²v
                -
                convection_y                # ∂v²/∂y
        ...
        for _ in range(N_PRESSURE_POISSION_ITERATIONS):
            ...
            # left where there is a homogeneous Dirichlet
