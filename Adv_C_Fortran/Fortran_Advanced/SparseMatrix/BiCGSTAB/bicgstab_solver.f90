...
program BiCGSTABSolver
    ...
    if (command_argument_count() < 3) then
        ...
        print *, 'Usage: ./bicgstab_solver <matrix dimension> <tolerance> <max iterations>'
        print *, 'Example: ./bicgstab_solver 4 1.0E-6 1000'
        stop
    end if
    ...
    function matvec(matA, vecX, NDIM) result(vecAx)
        ...
        do i = 1, NDIM
            do j = 1, NDIM
                vecAx(i) = vecAx(i) + matA(i,j) * vecX(j)
            end do
        end do
    end function matvec
...
! Alternative approach which, does not require explicit implementation.
subroutine BiCGSTAB_v2(A, b, x, n, tol, max_iter)
    implicit none
    integer, intent(in) :: n, max_iter
    real(8), intent(in) :: A(n,n), b(n), tol
    real(8), intent(out) :: x(n)
    real(8) :: r(n), r0(n), p(n), v(n), s(n), t(n)
    real(8) :: alpha, beta, omega, rho, rho_old, res_norm
    logical :: converged
    integer :: i, j, k
    ...
    ! Start the BiCGSTAB algorithm
    ! Perform r = b - A * x, directly within the solver
    r = b
    do i = 1, n
        do j = 1, n
            r(i) = r(i) - A(i,j) * x(j)
    ...
    do k = 1, max_iter
        ...
        ! Perform v = A * p directly within the solver
        v = 0.0
        do i = 1, n
            do j = 1, n
                v(i) = v(i) + A(i,j) * p(j)
        ...
        ! Perform t = A * s directly within the solver
        t = 0.0
        do i = 1, n
            do j = 1, n
                t(i) = t(i) + A(i,j) * s(j)
    ...
    if (converged) then
        ...
        print *, 'Result:'
        do i = 1, n
            write(*, '(A, I3, A, F12.8)') 'x(', i, ') = ', x(i)
        end do
    else
        print *, 'Failed to converge within', max_iter, 'iterations.'
    end if
end subroutine BiCGSTAB_v2
