...
!***end prologue  dqage
    ...
    epsilon = 1.0d-10  ! or another small value appropriate for your application
...
!           test on accuracy.
    ...
    if (ier /= 0 .OR. (abserr <= errbnd .AND. abs(abserr - resabs) > epsilon) &
    .OR. abs(abserr) < epsilon) go to 60
...
!           ------------
    ...
    !           and error and test for accuracy.
        ...
        if (abs(defab1 - error1) < epsilon .OR. abs(defab2 - error2) < epsilon) go to 5
...
!           the integral, and estimate the absolute error.
    ...
    if (abs(resasc) > epsilon .AND. abs(abserr) > epsilon) &
