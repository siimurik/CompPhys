...
!    $ gfortran dr2_radau5.f90 radau5.f radaua.f -o amp
!    $ ./amp
! * * * * * * * * * * * * * * * * * * * * * * * * *
! --- DRIVER FOR RADAU5 AT THE AMPLIFIER PROBLEM
! * * * * * * * * * * * * * * * * * * * * * * * * *
! USES radau5.f and radaua.f
PROGRAM main
    IMPLICIT REAL(kind=8) (A-H,O-Z)
    ! --- PARAMETERS FOR RADAU5 (BANDED JACOBIAN)
    ...
    IJAC = 1
    ! --- JACOBIAN IS A BANDED MATRIX (LOWER AND UPPER BAND-WIDTHS ARE 1, 2)
    MLJAC= 1
    MUJAC= 2
...
SUBROUTINE JBAMPL(N,X,Y,DFY,LDFY,RPAR,IPAR)
    ...
    IMPLICIT REAL(kind=8) (A-H,O-Z)
    REAL(kind=8) Y(N),DFY(LDFY,N),RPAR(15)
    UE    = RPAR(1)
    UB    = RPAR(2)
    UF    = RPAR(3)
    ALPHA = RPAR(4)
