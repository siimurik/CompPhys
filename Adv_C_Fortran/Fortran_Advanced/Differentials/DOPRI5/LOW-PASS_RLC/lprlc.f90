...
! $ gfortran lprlc.f90 dopri5.f90 -o lprlc
! $ ./lprlc
...
subroutine solout_rlc(nr, xold, t, y, n, con, icomp, nd, rpar, ipar, irtrn)
    ...
    if (first_call) then
        ...
        dt_out = rpar(2) / rpar(3) / 20.0_dp  ! τ/20 for good resolution
        ...
        do while (t >= tout)
            write(6, '(1X,F12.6,2F14.6,I8)') tout, &
                   contd5(1, tout, con, icomp, nd), &
                   contd5(2, tout, con, icomp, nd), nr-1
            tout = tout + dt_out
        end do
    endif
