...
def plot_static_orbit(filename='kep.dat'):
    ...
    try:
        ...
        t = data[:, 1]    # time (s)
        x = data[:, 2]    # x position (km)
        y = data[:, 3]    # y position (km)
        vx = data[:,4]   # x velocity (km/s)
        vy = data[:,5]   # y velocity (km/s)
        r = data[:, 6]    # distance from Earth center (km)
...
def create_animation(filename='kep.dat', interval=50, save_gif=False):
...
def plot_3d_trajectory(filename='kep.dat'):
...
if __name__ == "__main__":
    ...
    import os
    if not os.path.exists('kep.dat'):
        print("No kep.dat file found!")
