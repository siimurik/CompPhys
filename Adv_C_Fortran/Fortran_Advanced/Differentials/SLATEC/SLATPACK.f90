...
!   ESTIMATE ERRORS AT ORDERS K,K-1,K-2
    ...
    DO 265 L = 1,NEQN
        ...
        TEMP4 = YP(L) - PHI(L,1)
        !IF(KM2)265,260,255
        IF (KM2 .lt. 0) GOTO 265
        IF (KM2 .eq. 0) GOTO 260  
        IF (KM2 .gt. 0) GOTO 255
        255 ERKM2 = ERKM2 + ((PHI(L,KM1)+TEMP4)*TEMP3)**2
        260 ERKM1 = ERKM1 + ((PHI(L,K)+TEMP4)*TEMP3)**2
        ERK = ERK + (TEMP4*TEMP3)**2
    265 END DO
    !IF(KM2)280,275,270
    IF (KM2 .lt. 0) GOTO 280
    IF (KM2 .eq. 0) GOTO 275  
    IF (KM2 .gt. 0) GOTO 270
...
!   TEST IF ORDER SHOULD BE LOWERED
    ...
    !IF(KM2)299,290,285
    IF (KM2 .lt. 0) GOTO 299
    IF (KM2 .eq. 0) GOTO 290  
    IF (KM2 .gt. 0) GOTO 285
