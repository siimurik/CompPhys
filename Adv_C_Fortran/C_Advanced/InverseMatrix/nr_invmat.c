...
Compile and execute with
    $ gcc nr_invmat.c nrutil.c -o invmat
    $ ./invmat 

If not, then use the systemic linker in /usr/local/lib
	$ gcc nr_invmat.c -o invmat -lnrutil
	$ ./invmat
*/
#include <stdio.h>
#include <stdlib.h>
...
int main() {
    //int n = N; // Example: solving a 3x3 system
    float **a, **y, d, *col;
    int i, j, *indx;

    // Allocate memory for the matrix and vectors
    a = matrix(1, N, 1, N);    // Matrix a[1..n][1..n]
    y = matrix(1, N, 1, N);    // Inverse matrix y[1..n][1..n]
    col = vector(1, N);        // Vector col[1..n]
