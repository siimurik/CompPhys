...
cumulative_refrozen = np.cumsum(refrozen_hist)
...
plt.legend()
plt.grid(True)
plt.title('Liquid Water Content in Snow Layers')

# NEW: Water balance (melt, refreezing, runoff)
plt.figure(figsize=(10, 6))
plt.subplot(2,1,1)
plt.plot(days, melt_depth, label='Cumulative melt')
plt.plot(days, cumulative_refrozen / rho_w, label='Cumulative refrozen')
plt.plot(days, cumulative_runoff / rho_w, label='Cumulative runoff')
plt.ylabel('Water equivalent [m]')
plt.legend()
plt.grid(True)
plt.title('Cumulative Water Balance')

plt.subplot(2,1,2)
plt.plot(days, melt_rate_hist * 1000 * 3600, label='Melt rate')
plt.plot(days, refrozen_hist / dt * 1000 * 3600, label='Refreeze rate')
plt.plot(days, runoff_hist / dt * 1000 * 3600, label='Runoff rate')
plt.xlabel('Time [days]')
...
if USE_ADVANCED_INSULATION:
    ...
    ax1 = plt.gca()
    ax1.plot(days, k_eff_hist, 'b-', label='k_eff')
    ax1.set_ylabel('k_eff [W/mK]', color='b')
    ax1.tick_params(axis='y', labelcolor='b')
    ax1.grid(True)
    ax2 = ax1.twinx()
    ax2.plot(days, Rins_hist, 'r--', label='R_ins')
    ax2.set_ylabel('R_ins [m²K/W]', color='r')
    ...
    plt.figure(figsize=(8,5))
    ax1 = plt.gca()
    ax1.plot(days, W_hist, 'b-', label='W')
    ax1.set_ylabel('W [kg/m²]', color='b')
    ax1.tick_params(axis='y', labelcolor='b')
    ax1.grid(True)
    ax2 = ax1.twinx()
    ax2.plot(days, melt_depth, 'r--', label='Melt depth')
    ax2.set_ylabel('Melt depth [m]', color='r')
    ax2.tick_params(axis='y', labelcolor='r')
    plt.xlabel('Time [days]')
